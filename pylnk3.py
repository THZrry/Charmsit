...
# THZrry : supports python3.4
# i need to use an old version of python
import argparse
import ntpath
import os
import re
...
class Flags(object):
    ...
    def __init__(self, flag_names, flags_bytes=0):
        ...
        self._flags = dict([(name, False) for name in flag_names])
...
class RootEntry(object):
    ...
    def __init__(self, root):
        if root is not None:
            ...
            self.root = _ROOT_LOCATIONS.get(self.guid, "UNKNOWN {}".format(self.guid))
...
class UwpSubBlock:
    ...
    def __str__(self):
        string = 'UwpSubBlock {} ({}): {}'.format(self.name,hex(self.type),self.value)
        return string.strip()
...
class UwpMainBlock:
    ...
    def __init__(self, bytes=None, guid = None, blocks=None):
        self._data = bytes or b''
...
class ExtraData_IconEnvironmentDataBlock(object):
    ...
    def read(self, bytes):
        ...
        # self._size = read_int(buf)
        # self._signature = read_int(buf)
        self.target_ansi = buf.read(260).decode(sys.getdefaultencoding())
    ...
    def __str__(self):
        ...
        s = 'IconEnvironmentDataBlock\n TargetAnsi: {}\n TargetUnicode: {}'.format(target_ansi, target_unicode)
        return s
...
class ExtraData_EnvironmentVariableDataBlock(object):
    ...
    def __str__(self):
        target_ansi = self.target_ansi.replace('\x00', '')
        target_unicode = self.target_unicode.replace('\x00', '')
        s = 'EnvironmentVariableDataBlock\n TargetAnsi: {}\n TargetUnicode: {}'.format(target_ansi, target_unicode)
        return s
